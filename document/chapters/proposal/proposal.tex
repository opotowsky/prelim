...
However, the previous discussion excluded some detail for clarity. If the
training data set includes uncertainties, the values discussed will not just be
numbers, but probability distributions. 
While initially uncertainties will be ignored for a broader analysis, they
cannot be ignored for long in the nuclear forensics context. These are quite
important for obtaining some measure of confidence in the solution. 
Including the uncertainty broadens each value to a range of possible values
The methods for
calculating the necessary probability distributions are outlined below.

Here, we change the variables to directly represent probability densities, as in
Equation \ref{eq:rho}.  $C$ is a constant given by the marginal likelihood,
which can be ignored when calculating relative probabilities, and
$\boldsymbol{d}$ and $\boldsymbol{m}$ represent the training data set and model
parameters, respectively. Thus, $\rho(\boldsymbol{d}|\boldsymbol{m})$ is the
likelihood function, $\rho(\boldsymbol{m})$ is the prior probability density,
and $\rho(\boldsymbol{m}|\boldsymbol{d})$ is the posterior probability density.
\begin{equation}
\label{eq:rho}
  \rho(\boldsymbol{m}|\boldsymbol{d}) = C\ *\
  \rho(\boldsymbol{d}|\boldsymbol{m})\ *\ \rho(\boldsymbol{m})
\end{equation}
...
\begin{equation}
  P(\boldsymbol{m}) = \integral_{\boldsymbol{m}} \rho(d) dd
\end{equation}

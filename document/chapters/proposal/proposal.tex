...
It is important to be able to compare the methods proposed here against each
other and against other analagous methods. For example, Experiment 2 (Section
\ref{sec:exp2}) aims to predict reactor history values using simulated gamma
spectra, and \gls{INDEPTH} uses gamma spectra as inputs.  These can be directly
compared using the inverse problem structure introduced in Section
\ref{sec:inverse}.  A simplified application of Equation \ref{eq:bayes_words}
is discussed further in Section \ref{sec:invcompare}.  The methods for
calculating the necessary probability distributions are outlined below.

Here, we change the variables to directly represent probability densities
\todo{density=distribution?}, as in Equation \ref{eq:rho}.  $C$ is a constant
given by the marginal likelihood, which can be ignored when calculating
relative probabilities, and $\boldsymbol{d}$ and $\boldsymbol{m}$.  represent
the training data set and model parameters, respectively. Thus,
$\rho(\boldsymbol{d}|\boldsymbol{m})$ is the likelihood function,
$\rho(\boldsymbol{m}$ is the prior probability density, and
$\rho(\boldsymbol{m}|\boldsymbol{d})$ is the posterior probability density.
\begin{equation}
\label{eq:rho}
  \rho(\boldsymbol{m}|\boldsymbol{d}) = C \* \rho(\boldsymbol{d}|\boldsymbol{m}) \* \rho(\boldsymbol{m})
\end{equation}\todo{fix eq mult}

The likelihood function is obtained by a large set of forward problems, e.g.,
the database of \gls{SNF} recipes and parameters used to train the machine
learning algorithms.  First, a histogram \todo{is it histogram with continuous
data?  binned histo?} of the burnup will be plotted over the entire training
set.  The shape of this provides the likelihood distribution (likely Gaussian).
***Note: This makes it easy to assume something is known by slicing the input
to the histogram.
...
burnup predictions from a given statistical model will be \textbf{insert method}
